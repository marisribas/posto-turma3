...
window.blockly.js.blockly.MeuLocal.marcarPostos = function(top3) {
 var item, i, lista, meulocal, objPosto, x;
  ...
  for (i = 1; i_inc >= 0 ? i <= i_end : i >= i_end; i += i_inc) {
    objPosto = top3[(i - 1)];
    this.cronapi.maps.createMarker("mapLocal", i, this.cronapi.object.getProperty(objPosto, 'posto'), this.cronapi.maps.createLatLngPoint(this.cronapi.object.getProperty(objPosto, 'latitude'), this.cronapi.object.getProperty(objPosto, 'longitude')), 'img/fillingstation.png', [this.cronapi.object.getProperty(objPosto, 'posto'),'. Custokm: ',this.cronapi.object.getProperty(objPosto, 'custokm')].join(''), '');
  }
}
