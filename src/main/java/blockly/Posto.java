...
public static Var listaMenorCusto() throws Exception {
 return new Callable<Var>() {
   ...
   public Var call() throws Exception {
    dados = cronapi.database.Operations.query(Var.valueOf("app.entity.Abastecimento"),Var.valueOf("select a.posto.posto, a.posto.latitude, a.posto.longitude, SUM(a.valor)/SUM(a.km) from Abastecimento a  group by a.posto.posto, a.posto.latitude, a.posto.longitude  order by SUM(a.valor)/SUM(a.km) asc"));
